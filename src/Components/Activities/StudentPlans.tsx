...
// import ErrorBoundary from '../ErrorsHandling/ErrorBoundary';  // Adjust the import path accordinglyfrom '../../Error'sHandling/Boundary';
// import ComponentWithError from "../ErrorsHandling/ComponentWithError"; // Adjust the import path accordingly
...
import ComponentWithError from "../ErrorsHandling/ComponentWithError"; // Use default import
import PlanService from '../../Services/PlanService';
import StudentService from '../../Services/StudentService';
import IStudentModel from '../../Models/IStudentModel';
import IActivityModel from '../../Models/IActivityModel';
...
const StudentPlans = () => {
  ...
  // Add this state variable to track the delete plan confirmation dialog open/close state
  ...
  useEffect(() => {
    ...
    const fetchPlans = async () => {
      ...
      try {
        ...
        setError('Failed to fetch the plans. Please try again later.');
      }
      setLoading(false);
    };
  ...
  const handleAddPlan = async () => {
    try {
      if (newPlan) {
        ...
        // Clear the selected plan after a short delay
        setTimeout(() => {
          setNewPlan('');
        }, 100);
      ...
      setError('Failed to add plan to student. Please try again later.');
    }
  };

  // Handle deleting a plan from the student's plans
  const handleDeletePlan = (planId: string) => {
    // Open the delete plan confirmation dialog
    setDeleteDialogOpen(true);
    // Set the selected plan ID to be deleted
    ...
    try {
      ...
      setSuccessMessage('Plan removed successfully.');
      setOpenSnackbar(true);
    } catch (error) {
      setError('Failed to remove plan from student. Please try again later.');
    }
    setLoading(false);
  };
  ...
  if (error) {
    return (
      <ErrorBoundary>
        <ComponentWithError message={error} onRetry={() => setError(null)} />
      </ErrorBoundary>
    );
  }

  // If a success message exists, display the Snackbar
  if (successMessage) {
    return (
